...
为了完成本文，还遇到了`jekyll template`转义的蛋疼问题（由`jekyll`使用的`liquid template`引起），`to escape {{ "{{ this " }}}}`需要用：

    {{ "{{ this " }}}}

解决方案详见[迁移SAE wordpress的图片到jekyll](http://poetcoder.com/jekyll/2012/09/22/migrate-images-from-wordpress/)
